...
const resetPasswordRequest = async (req, res) => {
  try {
    ...
    if (existingVerification && existingVerification.expiresAt > new Date()) {
      return res.status(400).json({
        message: "Resat Password request already sent",
      });
    }
    ...
    await Verification.create({
      ...
      expiresAt: new Date(Date.now() + 30 * 60 * 60 * 1000), // 30 minutes from now
    });

    const resetPasswordLink = `${process.env.FRONTEND_URL}/reset-password?token=${resetPasswordToken}`;
    const emailBody = `<p>Click <a href="${resetPasswordLink}">here</a> to reset your password. This link is valid for 30 minutes.</p>`;
...
const verifyResatPasswordTokenAndResatPassword = async (req, res) => {
  try {
